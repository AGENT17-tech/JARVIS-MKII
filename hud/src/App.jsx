...
const BOOT_STEPS = [
  ...
  { key: 'ticker',     delay: 4100 },
  { key: 'ready',      delay: 4400 },
]

/* ─── WORLD MAP — reference-accurate style ───────────────────── */
const WorldMap = () => {
  ...
  useEffect(() => {
    ...
    // Detailed continent/country outlines — normalized 0-100 coords
    const regions = [
    ...
    const draw = () => {
      ...
      // Background grid — matches reference image exactly
      ctx.strokeStyle = 'rgba(0,160,255,0.10)'
      ctx.lineWidth = 0.5
      // Vertical lines
      ...
      // Region fills + borders
      regions.forEach((reg, ri) => {
        const cP = Math.sin(frame * 0.01 + ri * 0.4) * 0.08 + 0.92
        ctx.beginPath()
        reg.pts.forEach(([px, py], i) => {
          const x = (px / 100) * W, y = (py / 100) * H
          i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y)
        })
        ctx.closePath()
        if (reg.fill) {
          ctx.fillStyle = `rgba(0,100,200,${0.12 * cP})`
          ctx.fill()
        }
        ctx.shadowColor = 'rgba(0,200,255,0.5)'
        ctx.shadowBlur = 2
        ctx.strokeStyle = `rgba(0,200,255,${0.65 * cP})`
        ctx.lineWidth = 0.9
        ctx.stroke()
        ctx.shadowBlur = 0
      })
      ...
      ctx.lineWidth = 0.5
      for (let y = H * 0.1; y < H; y += H * 0.2) {
        ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke()
      }
      ctx.setLineDash([])

      // Cairo pulsing marker
      const mP = Math.sin(frame * 0.07) * 0.5 + 0.5
      for (let r = 1; r <= 3; r++) {
        ctx.beginPath()
        ctx.arc(cairoX, cairoY, 4 + mP * 10 * r / 3, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(0,255,200,${(0.6 - mP * 0.4) / r})`
        ctx.lineWidth = 0.8; ctx.stroke()
      }
      ctx.shadowColor = 'rgba(0,255,200,0.9)'; ctx.shadowBlur = 10
      ctx.beginPath(); ctx.arc(cairoX, cairoY, 3, 0, Math.PI * 2)
      ctx.fillStyle = '#00ffc8'; ctx.fill()
      ctx.shadowBlur = 0
      ctx.font = 'bold 8px "Share Tech Mono"'
      ctx.fillStyle = `rgba(0,255,200,${0.9 * pulse})`
      ctx.fillText('◈ CAIRO', cairoX + 8, cairoY - 5)

      // City nodes + data packets traveling to Cairo
      const cities = [
        { x: 0.14, y: 0.25, n: 'NEW YORK' },
        { x: 0.40, y: 0.18, n: 'LONDON'   },
        { x: 0.58, y: 0.16, n: 'MOSCOW'   },
        { x: 0.70, y: 0.28, n: 'BEIJING'  },
        { x: 0.50, y: 0.55, n: 'NAIROBI'  },
        { x: 0.20, y: 0.55, n: 'SAO PAULO'},
      ]
      cities.forEach((city, ci) => {
        const a = Math.sin(frame * 0.03 + ci * 1.1) * 0.2 + 0.35
        const cx2 = city.x * W, cy2 = city.y * H
        // Dashed line to Cairo
        ctx.setLineDash([3, 6])
        ctx.beginPath(); ctx.moveTo(cx2, cy2); ctx.lineTo(cairoX, cairoY)
        ctx.strokeStyle = `rgba(0,160,255,${a * 0.45})`
        ctx.lineWidth = 0.5; ctx.stroke()
        ctx.setLineDash([])
        // Traveling packet
        const t = ((frame * 0.006 + ci * 0.18) % 1)
        const px = cx2 + (cairoX - cx2) * t
        const py = cy2 + (cairoY - cy2) * t
        ctx.beginPath(); ctx.arc(px, py, 1.5, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(0,255,200,${a * 1.8})`; ctx.fill()
        // Node dot
        ctx.beginPath(); ctx.arc(cx2, cy2, 2.5, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(0,210,255,${a * 2})`; ctx.fill()
        ctx.font = '7px "Share Tech Mono"'
        ctx.fillStyle = `rgba(0,200,255,${a * 1.3})`
        ctx.fillText(city.n, cx2 - 12, cy2 - 6)
      })
      ...
      sg.addColorStop(0.5, `rgba(0,200,255,${0.14 * pulse2})`)
      sg.addColorStop(1, 'rgba(0,200,255,0)')
      ctx.fillStyle = sg; ctx.fillRect(0, scanY - 10, W, 20)

      // Corner data readouts (like reference image)
      ctx.font = '8px "Share Tech Mono"'
      ctx.fillStyle = `rgba(0,180,255,${0.5 * pulse})`
      ctx.fillText('GLOBAL NETWORK — LIVE', 8, H - 5)
      ctx.fillText(`FRAME: ${String(frame).padStart(6, '0')}`, W - 100, H - 5)
      ctx.fillText('LAT: 30.04° N  LON: 31.24° E', W / 2 - 80, H - 5)

      // Top-left corner coords (like reference)
      ctx.fillStyle = `rgba(0,160,255,${0.4 * pulse})`
      ctx.fillText('90°N', 6, 14)
      ctx.fillText('90°S', 6, H - 14)
      ctx.fillText('180°W', 6, H / 2)
      ctx.fillText('180°E', W - 36, H / 2)
    }
...
/* ─── IRON MAN SUIT — MCU hologram style ─────────────────────── */
// Reference: white-teal translucent hologram, light radiates outward,
// body glows from within, minimal harsh lines, ethereal/projected light feel
const IronManCanvas = ({ active, rotY }) => {
  ...
  useEffect(() => {
    ...
    // MCU hologram palette: whites, cyans, teals — radiant not outlined
    const glow = (x, y, r, alpha, color = [0, 220, 255]) =>
      (()=>{
    ...
    const seg = (pts, glowAlpha = 0.7, fillAlpha = 0.08, color = '0,220,255', glowColor = '0,200,255') => {
      ctx.beginPath()
      pts.forEach(([x, y], i) => i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y))
      ctx.closePath()
      ctx.fillStyle = `rgba(${color},${fillAlpha})`; ctx.fill()
      ctx.shadowColor = `rgba(${glowColor},0.8)`; ctx.shadowBlur = 8
      ctx.strokeStyle = `rgba(${color},${glowAlpha})`; ctx.lineWidth = 1.2; ctx.stroke()
      ctx.shadowBlur = 0
    }
    ...
    const draw = () => {
      ...
      const skew = Math.sin((rot * Math.PI) / 180) * 0.08
      const sx = Math.abs(Math.cos((rot * Math.PI) / 180)) * 0.18 + 0.82
      const pulse = Math.sin(frame * 0.04) * 0.2 + 0.8
      const breathe = Math.sin(frame * 0.025) * 0.05 + 0.95
      const arcPulse = speak ? pulse : 0.7

      ctx.save(); ctx.translate(cx, 8); ctx.transform(sx, 0, skew, 1, 0, 0); ctx.translate(-cx, 0)

      // AMBIENT BODY GLOW — the hologram radiates light outward
      const bodyGrad = ctx.createRadialGradient(cx, H * 0.42, 10, cx, H * 0.42, W * 0.55)
      bodyGrad.addColorStop(0, `rgba(0,180,255,${0.06 * breathe})`)
      bodyGrad.addColorStop(0.5, `rgba(0,160,255,${0.03 * breathe})`)
      bodyGrad.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = bodyGrad
      ctx.fillRect(0, 0, W, H)

      // COLOR PALETTE: white-cyan for bright parts, teal for mid, cyan for edges
      const WHITE  = (a) => `rgba(200,240,255,${a})`
      const CYAN   = (a) => `rgba(0,220,255,${a})`
      const TEAL   = (a) => `rgba(0,255,200,${a})`
      const PALE   = (a) => `rgba(160,230,255,${a})`

      const S = (pts, bA = 0.8, fA = 0.1, c = '160,230,255') => seg(pts, bA * breathe, fA * breathe, c)
      const ln = (x1, y1, x2, y2, a = 0.3, w = 0.6) => {
        ctx.beginPath(); ctx.moveTo(x1, y1); ctx.lineTo(x2, y2)
        ctx.strokeStyle = `rgba(160,230,255,${a})`; ctx.lineWidth = w; ctx.stroke()
      }

      // HELMET — angular MCU faceplate
      // Main helmet shell
      ...
      // Jaw line detail
      ln(cx-26,30,cx+26,30, 0.4, 0.7)
      ln(cx-26,38,cx+26,38, 0.3, 0.5)
      ln(cx,7,cx,54, 0.25, 0.5)
      // EYES — bright white-cyan, most intense part
      const eyeP = speak ? pulse : 0.88
      ctx.shadowColor = 'rgba(150,240,255,0.95)'; ctx.shadowBlur = speak ? 20 : 14
      // Left eye
      ctx.beginPath(); ctx.moveTo(cx-22,35); ctx.lineTo(cx-16,32); ctx.lineTo(cx-10,34); ctx.lineTo(cx-12,39); ctx.lineTo(cx-18,41); ctx.closePath()
      ctx.fillStyle = `rgba(200,248,255,${eyeP})`; ctx.fill()
      // Right eye
      ctx.beginPath(); ctx.moveTo(cx+22,35); ctx.lineTo(cx+16,32); ctx.lineTo(cx+10,34); ctx.lineTo(cx+12,39); ctx.lineTo(cx+18,41); ctx.closePath()
      ctx.fillStyle = `rgba(200,248,255,${eyeP})`; ctx.fill()
      ctx.shadowBlur = 0
      // Eye ambient glow
      glow(cx - 16, 36, 14, eyeP * 0.6, '0,210,255')
      glow(cx + 16, 36, 14, eyeP * 0.6, '0,210,255')
      ...
      S([[cx+42,64],[cx+44,78],[cx+36,86],[cx,84],[cx,64],[cx+28,60]], 0.55, 0.08, '130,215,255')
      // Chest detail lines
      ln(cx,58,cx,118, 0.3, 0.5)
      ln(cx-42,72,cx+42,72, 0.25, 0.5)
      ln(cx-44,86,cx+44,86, 0.2, 0.45)
      ln(cx-42,100,cx+42,100, 0.18, 0.4)
      ln(cx-40,110,cx+40,110, 0.18, 0.4)
      ctx.beginPath(); ctx.moveTo(cx-40,64); ctx.quadraticCurveTo(cx,60,cx+40,64)
      ctx.strokeStyle='rgba(180,240,255,0.5)'; ctx.lineWidth=0.8; ctx.stroke()

      // ARC REACTOR — brightest element, white core
      const ary = 84
      // Outer ambient glow
      glow(cx, ary, 28, arcPulse * 0.9, '0,200,255')
      // Rotating tick marks
      for (let i = 0; i < 12; i++) {
        const ang = (i / 12) * Math.PI * 2 + frame * 0.025
        const r1 = 17, r2 = 22
        ctx.beginPath()
        ctx.moveTo(cx + Math.cos(ang) * r1, ary + Math.sin(ang) * r1)
        ctx.lineTo(cx + Math.cos(ang) * r2, ary + Math.sin(ang) * r2)
        ctx.shadowColor = 'rgba(0,230,255,0.8)'; ctx.shadowBlur = 4
        ctx.strokeStyle = i % 3 === 0 ? `rgba(200,248,255,${0.9*arcPulse})` : `rgba(0,220,255,${0.4*arcPulse})`
        ctx.lineWidth = i % 3 === 0 ? 1.8 : 0.8; ctx.stroke()
        ctx.shadowBlur = 0
      }
      ctx.beginPath(); ctx.arc(cx, ary, 16, 0, Math.PI * 2)
      ctx.shadowColor = 'rgba(0,230,255,0.8)'; ctx.shadowBlur = 6
      ctx.strokeStyle = `rgba(180,240,255,${0.9*arcPulse})`; ctx.lineWidth = 1.5; ctx.stroke()
      ctx.beginPath(); ctx.arc(cx, ary, 11, 0, Math.PI * 2)
      ctx.strokeStyle = `rgba(0,255,200,${0.8*arcPulse})`; ctx.lineWidth = 1; ctx.stroke()
      ctx.beginPath(); ctx.arc(cx, ary, 6, 0, Math.PI * 2)
      ctx.strokeStyle = `rgba(200,248,255,${arcPulse})`; ctx.lineWidth = 0.9; ctx.stroke()
      ctx.shadowBlur = 0
      // Core white orb — like reference image
      ctx.shadowColor = 'rgba(200,248,255,0.95)'; ctx.shadowBlur = speak ? 24 : 16
      const cg = ctx.createRadialGradient(cx, ary, 0, cx, ary, 5)
      cg.addColorStop(0, 'rgba(240,252,255,1)')
      cg.addColorStop(0.6, `rgba(0,230,255,${arcPulse})`)
      cg.addColorStop(1, 'rgba(0,180,255,0)')
      ctx.fillStyle = cg; ctx.beginPath(); ctx.arc(cx, ary, 5, 0, Math.PI * 2); ctx.fill()
      ctx.shadowBlur = 0
      // Spokes
      for (let a = 0; a < 8; a++) {
        const ang = a * Math.PI / 4 + frame * 0.012
        ctx.beginPath()
        ctx.moveTo(cx + Math.cos(ang) * 6, ary + Math.sin(ang) * 6)
        ctx.lineTo(cx + Math.cos(ang) * 13, ary + Math.sin(ang) * 13)
        ctx.strokeStyle = `rgba(180,240,255,${0.6*arcPulse})`; ctx.lineWidth = 0.7; ctx.stroke()
      }
      ...
      for (let r = 0; r < 3; r++) for (let c = 0; c < 2; c++) {
        ctx.beginPath(); ctx.arc(cx - 58 + c * 6, 150 + r * 8, 1.4, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(160,230,255,0.45)`; ctx.fill()
      }
      S([[cx+62,138],[cx+64,148],[cx+64,166],[cx+60,174],[cx+52,176],[cx+46,172],[cx+44,162],[cx+44,138]], 0.70, 0.09, '150,225,255')
      ln(cx+64,148,cx+46,148, 0.22, 0.4); ln(cx+64,158,cx+46,158, 0.2, 0.4); ln(cx+62,168,cx+48,168, 0.2, 0.4)
      for (let r = 0; r < 3; r++) for (let c = 0; c < 2; c++) {
        ctx.beginPath(); ctx.arc(cx + 52 + c * 6, 150 + r * 8, 1.4, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(160,230,255,0.45)`; ctx.fill()
      }

      // GAUNTLETS — repulsor glow
      ...
      ctx.beginPath(); ctx.arc(cx - 52, 192, 7, 0, Math.PI * 2)
      ctx.strokeStyle = `rgba(0,255,200,${speak ? 0.95 : 0.6})`; ctx.lineWidth = 1.3; ctx.stroke()
      ctx.beginPath(); ctx.arc(cx - 52, 192, 3.5, 0, Math.PI * 2)
      ctx.fillStyle = `rgba(180,255,240,${speak ? 0.95 : 0.5})`; ctx.fill()
      ctx.shadowBlur = 0
      S([[cx+60,176],[cx+62,182],[cx+60,190],[cx+54,194],[cx+46,192],[cx+42,186],[cx+42,176],[cx+52,174]], 0.80, 0.11, '160,230,255')
      glow(cx + 52, 192, 10, speak ? arcPulse * 0.9 : 0.3, '0,200,255')
      ctx.shadowColor = speak ? 'rgba(0,255,200,0.9)' : 'rgba(0,200,255,0.4)'; ctx.shadowBlur = speak ? 18 : 7
      ...
      // ABDOMEN — hexagonal tile detail
      S([[cx-36,114],[cx-38,122],[cx-38,140],[cx-34,148],[cx,150],[cx+34,148],[cx+38,140],[cx+38,122],[cx+36,114],[cx,118]], 0.75, 0.10, '150,225,255')
      const tiles = [
        [cx-26,118],[cx-12,118],[cx+2,118],[cx+16,118],
        [cx-20,128],[cx-6,128],[cx+8,128],[cx+22,128],
        [cx-26,138],[cx-12,138],[cx+2,138],[cx+16,138],
      ]
      tiles.forEach(([tx, ty]) => {
        const hw = 6, hh = 4
        ctx.beginPath()
        ctx.moveTo(tx, ty-hh); ctx.lineTo(tx+hw, ty-hh/2); ctx.lineTo(tx+hw, ty+hh/2)
        ctx.lineTo(tx, ty+hh); ctx.lineTo(tx-hw, ty+hh/2); ctx.lineTo(tx-hw, ty-hh/2)
        ctx.closePath()
        ctx.strokeStyle = 'rgba(160,230,255,0.25)'; ctx.lineWidth = 0.4; ctx.stroke()
      })
      ...
      for (let r = 0; r < 4; r++) {
        ctx.beginPath(); ctx.arc(cx - 22, 180 + r * 10, 1.5, 0, Math.PI * 2)
        ctx.fillStyle = 'rgba(160,230,255,0.4)'; ctx.fill()
      }
      S([[cx+34,168],[cx+36,178],[cx+36,200],[cx+32,216],[cx+24,220],[cx+16,218],[cx+12,208],[cx+12,188],[cx+16,174],[cx+24,168]], 0.72, 0.10, '150,225,255')
      ln(cx+36,182,cx+14,180, 0.22, 0.4); ln(cx+36,194,cx+14,192, 0.2, 0.4); ln(cx+36,206,cx+14,204, 0.2, 0.4)
      for (let r = 0; r < 4; r++) {
      ...
      ctx.beginPath(); ctx.arc(cx - 24, 228, 7, 0, Math.PI * 2)
      ctx.strokeStyle = 'rgba(180,240,255,0.5)'; ctx.lineWidth = 0.8; ctx.stroke()
      S([[cx+36,216],[cx+38,224],[cx+34,234],[cx+26,238],[cx+16,236],[cx+12,228],[cx+12,218],[cx+16,214],[cx+26,214]], 0.82, 0.12, '170,235,255')
      ctx.beginPath(); ctx.arc(cx + 24, 228, 7, 0, Math.PI * 2)
      ctx.strokeStyle = 'rgba(180,240,255,0.5)'; ctx.lineWidth = 0.8; ctx.stroke()
      ...
      ctx.beginPath(); ctx.ellipse(cx - 23, 306, 11, 4, 0, 0, Math.PI * 2)
      ctx.strokeStyle = `rgba(0,255,200,${speak ? 0.9 : 0.5})`; ctx.lineWidth = 1; ctx.stroke()
      ctx.beginPath(); ctx.ellipse(cx - 23, 306, 6, 2, 0, 0, Math.PI * 2)
      ctx.fillStyle = `rgba(180,255,240,${speak ? 0.7 : 0.25})`; ctx.fill()
      ctx.shadowBlur = 0
      S([[cx+34,286],[cx+36,294],[cx+34,302],[cx+26,308],[cx+16,308],[cx+10,302],[cx+10,290],[cx+12,284]], 0.78, 0.11, '160,230,255')
      ctx.shadowColor = speak ? 'rgba(0,255,200,0.8)' : 'rgba(0,200,255,0.3)'; ctx.shadowBlur = speak ? 16 : 5
      ctx.beginPath(); ctx.ellipse(cx + 23, 306, 11, 4, 0, 0, Math.PI * 2)
      ...
      // BASE PLATFORM GLOW
      const bg = ctx.createRadialGradient(cx, 314, 0, cx, 314, 60)
      bg.addColorStop(0, `rgba(0,200,255,${0.4 * pulse})`)
      bg.addColorStop(1, 'rgba(0,0,0,0)')
      ctx.fillStyle = bg
      ctx.beginPath(); ctx.ellipse(cx, 314, 60, 14, 0, 0, Math.PI * 2); ctx.fill()
...
const CairoHologram = () => {
  ...
  useEffect(() => {
    ...
    const buildings = [
      {x:10,w:30,h:60,type:'pyramid'},{x:50,w:20,h:90,type:'tower'},
      {x:78,w:35,h:50,type:'pyramid'},{x:120,w:15,h:110,type:'minaret'},
      {x:142,w:28,h:70,type:'tower'},{x:178,w:12,h:95,type:'minaret'},
      {x:198,w:40,h:55,type:'pyramid'},{x:246,w:18,h:85,type:'tower'},
      {x:272,w:25,h:65,type:'tower'},{x:305,w:14,h:100,type:'minaret'},
      {x:328,w:32,h:58,type:'pyramid'},{x:368,w:16,h:88,type:'tower'},
      {x:392,w:22,h:72,type:'tower'},{x:422,w:10,h:105,type:'minaret'},
      {x:440,w:36,h:48,type:'pyramid'},{x:484,w:20,h:80,type:'tower'},
    ]
    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height); frame++
      const baseY = canvas.height - 20
      const pulse = Math.sin(frame * 0.03) * 0.25 + 0.75
      const pulse2 = Math.sin(frame * 0.05 + 1) * 0.18 + 0.82
      ctx.strokeStyle = 'rgba(0,212,255,0.12)'; ctx.lineWidth = 0.5
      for (let x = 0; x < canvas.width; x += 20) {
        ctx.beginPath(); ctx.moveTo(x, baseY); ctx.lineTo(canvas.width/2+(x-canvas.width/2)*0.28, baseY-20); ctx.stroke()
      }
      for (let i = 0; i < 6; i++) {
        const y = baseY - i * 4; ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke()
      }
      buildings.forEach((b, idx) => {
        const bP = Math.sin(frame * 0.04 + idx * 0.5) * 0.15 + 0.85, top = baseY - b.h
        if (b.type === 'pyramid') {
          ctx.shadowColor = 'rgba(0,212,255,0.6)'; ctx.shadowBlur = 4
          ctx.beginPath(); ctx.moveTo(b.x+b.w/2,top); ctx.lineTo(b.x+b.w,baseY); ctx.lineTo(b.x,baseY); ctx.closePath()
          ctx.strokeStyle = `rgba(0,212,255,${0.85*bP})`; ctx.lineWidth = 1.2; ctx.stroke()
          ctx.fillStyle = 'rgba(0,200,255,0.07)'; ctx.fill(); ctx.shadowBlur = 0
        } else if (b.type === 'minaret') {
          ctx.shadowColor = 'rgba(0,212,255,0.5)'; ctx.shadowBlur = 3
          ctx.strokeStyle = `rgba(0,212,255,${0.80*bP})`; ctx.lineWidth = 1.1
          ctx.strokeRect(b.x+b.w*0.3, top+b.h*0.3, b.w*0.4, b.h*0.7)
          ctx.beginPath(); ctx.arc(b.x+b.w/2, top+b.h*0.3, b.w*0.35, Math.PI, 0); ctx.stroke()
          ctx.beginPath(); ctx.moveTo(b.x+b.w/2,top); ctx.lineTo(b.x+b.w/2,top+b.h*0.1)
          ctx.strokeStyle = `rgba(0,255,200,${0.95*bP})`; ctx.lineWidth = 1.8; ctx.stroke()
          ctx.shadowBlur = 0
          ctx.strokeStyle = `rgba(0,212,255,${0.65*bP})`; ctx.lineWidth = 1; ctx.strokeRect(b.x, baseY-b.h*0.12, b.w, b.h*0.12)
        } else {
          ctx.shadowColor = 'rgba(0,212,255,0.4)'; ctx.shadowBlur = 3
          ctx.strokeStyle = `rgba(0,212,255,${0.80*bP})`; ctx.lineWidth = 1.1
          ctx.strokeRect(b.x, top, b.w, b.h); ctx.fillStyle = 'rgba(0,212,255,0.05)'; ctx.fillRect(b.x, top, b.w, b.h)
          ctx.shadowBlur = 0
          const cols = Math.max(2, Math.floor(b.w/6)), rows = Math.max(3, Math.floor(b.h/10))
          for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
            if (Math.sin(frame*0.02 + idx*1.3 + r*0.7 + c*1.1) > 0.35) {
              ctx.fillStyle = `rgba(0,255,180,${0.45*bP})`
              ctx.fillRect(b.x+2+c*(b.w-4)/cols, top+4+r*(b.h-8)/rows, (b.w-4)/cols-1, (b.h-8)/rows-2)
            }
          }
          if (b.h > 75) {
            ctx.beginPath(); ctx.moveTo(b.x+b.w/2,top); ctx.lineTo(b.x+b.w/2,top-12)
            ctx.strokeStyle = `rgba(0,255,200,${0.95*bP})`; ctx.lineWidth = 1.4; ctx.stroke()
            ctx.shadowColor = 'rgba(255,60,60,0.8)'; ctx.shadowBlur = 6
            ctx.beginPath(); ctx.arc(b.x+b.w/2, top-12, 2.2, 0, Math.PI*2)
            ctx.fillStyle = `rgba(255,60,60,${bP})`; ctx.fill(); ctx.shadowBlur = 0
          }
        }
      })
      const scanX = ((frame*2) % (canvas.width+40)) - 20
      const sg = ctx.createLinearGradient(scanX-15, 0, scanX+15, 0)
      sg.addColorStop(0,'rgba(0,212,255,0)'); sg.addColorStop(0.5,'rgba(0,212,255,0.28)'); sg.addColorStop(1,'rgba(0,212,255,0)')
      ctx.fillStyle = sg; ctx.fillRect(scanX-15, 0, 30, canvas.height)
      const gg = ctx.createLinearGradient(0, baseY, canvas.width, baseY)
      gg.addColorStop(0,'rgba(0,212,255,0)'); gg.addColorStop(0.3,`rgba(0,212,255,${0.65*pulse2})`); gg.addColorStop(0.7,`rgba(0,212,255,${0.65*pulse2})`); gg.addColorStop(1,'rgba(0,212,255,0)')
      ctx.shadowColor = 'rgba(0,212,255,0.6)'; ctx.shadowBlur = 6
      ctx.strokeStyle = gg; ctx.lineWidth = 1.8; ctx.beginPath(); ctx.moveTo(0,baseY); ctx.lineTo(canvas.width,baseY); ctx.stroke()
      ctx.shadowBlur = 0
      ctx.font = '9px "Share Tech Mono",monospace'; ctx.fillStyle = `rgba(0,212,255,${0.65*pulse})`
      ctx.fillText('◈ CAIRO, EG  —  30.0444° N  31.2357° E', 10, canvas.height - 4)
    }
...
const AlertTicker = ({ visible }) => {
  const items = [
    '◈ ALL SYSTEMS NOMINAL','⬡ POWER CORE: 100%','◈ PERIMETER: SECURE',
    '⬡ UPLINK: ESTABLISHED','◈ AI CORE: ONLINE','⬡ THREAT LEVEL: MINIMAL',
    '◈ MEMORY BANKS: ACTIVE','⬡ ENCRYPTION: AES-256','◈ NETWORK: SECURE',
    '⬡ FIRMWARE: CURRENT','◈ BIOMETRICS: VERIFIED','⬡ AGENT17-TECH AUTH OK',
  ]
...
const S = {
  ...
  panel:{
    position:'fixed',background:'rgba(0,7,22,0.88)',border:'1px solid rgba(0,212,255,0.2)',
    borderRadius:3,padding:'14px 18px',width:274,
    backdropFilter:'blur(6px)',boxShadow:'0 0 22px rgba(0,80,180,0.08),inset 0 1px 0 rgba(0,212,255,0.07)',
  },
...
const HUD = () => {
        ...
        if (key === 'ready') {
          ...
          ws.onmessage = (e) => {
            ...
            if (e.data.startsWith('voice:response:')) {
              ...
              if (jid) {
                setMessages(prev => prev.map(m => m.id === jid ? { ...m, text } : m))
              } else {
                setMessages(prev => [...prev, { id: Date.now(), text, type: 'jarvis' }])
              }
              setIsThinking(false)
          ...
          fetch('http://localhost:8000/greeting').then(r => r.json()).then(data => {
            ...
            if (!hasGreeted.current) {
              ...
              fetch('http://localhost:8000/speak', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ text:data.greeting }) }).catch(() => {})
            }
  ...
  useEffect(() => {
    const fw = () => fetch('http://localhost:8000/weather').then(r=>r.json()).then(d=>{ if(!d.error) setWeather(d) }).catch(()=>{})
    fw(); const id = setInterval(fw, 600000); return () => clearInterval(id)
  }, [])
  useEffect(() => {
    const fg = () => fetch('http://localhost:8000/github').then(r=>r.json()).then(d=>{ if(Array.isArray(d)) setRepos(d) }).catch(()=>{})
    fg(); const id = setInterval(fg, 5*60*1000); return () => clearInterval(id)
  }, [])
  ...
  const scanScreen = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/chat', {
        ...
        body:JSON.stringify({ content:'What do you see on my screen right now?' })
      })
      const reader = res.body.getReader(), dec = new TextDecoder()
      while (true) { const { done, value } = await reader.read(); if (done) break; const tok = dec.decode(value); setMessages(prev => prev.map(m => m.id===jid ? {...m,text:m.text+tok} : m)) }
  ...
  const rings = [
    { ...rb, width:410,height:410,
      border:`1.5px solid rgba(0,180,255,${isSpeaking?0.88:0.36})`,
      borderTopColor:`rgba(0,230,255,${isSpeaking?1:0.9})`,
      boxShadow:isSpeaking?'0 0 45px rgba(0,212,255,0.65),0 0 90px rgba(0,180,255,0.3)':'0 0 20px rgba(0,180,255,0.22)',
      animationName:'spin1',animationTimingFunction:'linear',animationIterationCount:'infinite',
      animationDuration:isThinking?'0.9s':isSpeaking?'0.45s':'4.2s',
      transform:isSpeaking?'scale(1.06)':'scale(1)',transition:'transform 0.4s ease,box-shadow 0.4s ease,border-color 0.4s ease'},
    { ...rb, width:332,height:332,
      border:'1px solid rgba(0,155,255,0.2)',
      borderBottomColor:`rgba(0,212,255,${isSpeaking?0.88:0.55})`,
      animationName:'spin2',animationTimingFunction:'linear',animationIterationCount:'infinite',
      animationDuration:isThinking?'0.7s':isSpeaking?'0.35s':'3.1s',
      transform:isSpeaking?'scale(1.05)':'scale(1)',transition:'transform 0.4s ease'},
    { ...rb, width:256,height:256,
      border:'1px solid rgba(0,155,255,0.26)',
      borderLeftColor:`rgba(0,200,255,${isSpeaking?0.82:0.6})`,
      borderRightColor:`rgba(0,200,255,${isSpeaking?0.82:0.6})`,
      animationName:'spin1',animationTimingFunction:'linear',animationIterationCount:'infinite',
      animationDuration:isThinking?'1.2s':isSpeaking?'0.6s':'6s',
      transform:isSpeaking?'scale(1.04)':'scale(1)',transition:'transform 0.4s ease'},
    { ...rb, width:182,height:182,
      border:'1px solid rgba(0,155,255,0.16)',
      borderTopColor:`rgba(0,255,200,${isSpeaking?0.72:0.44})`,
      animationName:'spin2',animationTimingFunction:'linear',animationIterationCount:'infinite',
      animationDuration:isThinking?'1.8s':isSpeaking?'0.8s':'8s'},
    { ...rb, width:122,height:122,
      border:`1px solid rgba(0,255,200,${isSpeaking?0.55:0.2})`,
      borderBottomColor:`rgba(0,255,200,${isSpeaking?0.92:0.48})`,
      animationName:'spin1',animationTimingFunction:'linear',animationIterationCount:'infinite',
      animationDuration:isThinking?'1s':isSpeaking?'0.5s':'5s',
      transform:isSpeaking?'scale(1.06)':'scale(1)',transition:'transform 0.4s ease'},
  ]
  ...
  return (
    <div style={{...S.root,filter:shutStep>=7?'brightness(0)':shutStep>=6?'brightness(0.12)':'none',transition:'filter 0.9s ease'}}>
      ...
      {/* POWER BUTTON */}
      {booted && !shutting && (
        <div onClick={triggerShutdown} title="Shutdown JARVIS" style={{position:'fixed',top:14,right:14,width:28,height:28,borderRadius:'50%',border:'1px solid rgba(255,60,60,0.38)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',zIndex:200,background:'rgba(255,30,30,0.07)',transition:'all 0.25s ease'}}
      ...
      <div style={{...S.panel,...S.topLeft,transition:'transform 0.7s cubic-bezier(0.16,1,0.3,1),opacity 0.7s ease',transform:boot.panelTL?'translateX(0)':'translateX(-130px)',opacity:shutStep>=1?0:boot.panelTL?1:0}}>
        <div style={S.panelTitle}>SYSTEM STATUS</div>
        <StatBar label="CPU"  value={boot.stats?stats.cpu:0}  color="#00d4ff"/>
        <StatBar label="RAM"  value={boot.stats?stats.ram:0}  color="#00ffc8"/>
        <StatBar label="TEMP" value={boot.stats?stats.temp:0} max={100} color="#ffb900" unit="°C"/>
        <div style={S.divider}/>
        <div style={S.statRow}><span style={S.dimText}>JARVIS CORE</span><span style={{...S.valueText,color:'#00ffc8'}}>ONLINE</span></div>
        <div style={S.statRow}><span style={S.dimText}>NODE</span><span style={S.valueText}>ONLINE</span></div>
        <div style={S.statRow}><span style={S.dimText}>NETWORK</span><span style={{...S.valueText,color:'#00ffc8'}}>SECURE</span></div>
      </div>
      ...
      {/* ── LEFT — SUIT (MARK L SCHEMATIC) ── */}
      <div style={{
        position:'fixed',left:36,top:310,width:282,
        display:'flex',flexDirection:'column',alignItems:'center',
        transition:'opacity 0.9s ease,transform 0.9s cubic-bezier(0.16,1,0.3,1)',
        opacity:shutStep>=3?0:boot.suit?1:0,
        transform:boot.suit?'translateX(0)':'translateX(-130px)',
      }}>
      ...
      {/* ── BOTTOM RIGHT — JARVIS INTERFACE ── */}
      <div style={{
        ...S.panel,
        position:'fixed',bottom:290,right:36,width:354,
        transition:'transform 0.7s cubic-bezier(0.16,1,0.3,1),opacity 0.7s ease',
        transform:boot.panelBR?'translateX(0)':'translateX(130px)',
        opacity:shutStep>=1?0:boot.panelBR?1:0,
      }}>
        ...
        <div style={S.inputRow}>
          ...
          <button
            ...
            title="Scan screen"
            style={{
              background:'none',border:'1px solid rgba(0,212,255,0.25)',borderRadius:3,
              color:isThinking?'rgba(0,212,255,0.2)':'rgba(0,212,255,0.6)',
              fontFamily:'Share Tech Mono',fontSize:9,letterSpacing:1,
              padding:'4px 7px',cursor:isThinking?'not-allowed':'pointer',
              flexShrink:0,transition:'all 0.2s ease',
              textShadow:isThinking?'none':'0 0 6px rgba(0,212,255,0.4)',
            }}
      ...
      {/* ── BOTTOM RIGHT — MISSION LOG (below JARVIS interface) ── */}
      <div style={{
        ...S.panel,
        position:'fixed',bottom:100,right:36,width:354,
        transition:'transform 0.7s cubic-bezier(0.16,1,0.3,1),opacity 0.7s ease',
        transform:boot.missionlog?'translateX(0)':'translateX(130px)',
        opacity:shutStep>=2?0:boot.missionlog?1:0,
      }}>
        <div style={S.panelTitle}>MISSION LOG</div>
        <div style={S.logList}>
          {[
            {t:'JARVIS Core — Phase 2 active', s:'active'},
            {t:'HUD Interface — Online',        s:'active'},
            {t:'FastAPI Backend — Online',       s:'active'},
            {t:'Persistent Memory — Online',     s:'active'},
            {t:'Voice Integration — Online',     s:'active'},
            {t:'Vision — Online',                  s:'active'},
          ].map((task, i) => (
      ...
      {/* ── GITHUB PANEL ── */}
      <div style={{...S.panel,position:'fixed',bottom:52,left:36,width:390,transition:'opacity 0.9s ease,transform 0.9s ease',opacity:shutStep>=2?0:boot.github?1:0,transform:boot.github?'translateY(0)':'translateY(20px)',zIndex:10,padding:'14px 18px'}}>
        {/* Header */}
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          <div style={S.panelTitle}>AGENT17-TECH // GITHUB</div>
          <div style={{fontFamily:'Share Tech Mono',fontSize:9,color:'#00ffc8',letterSpacing:1.5}}>
            {repos.length>0 ? `${repos.length} REPOS` : '—'}
          </div>
        </div>

        {repos.length===0 ? (
          <div style={{...S.dimText,textAlign:'center',padding:'10px 0'}}>FETCHING REPOSITORIES...</div>
        ) : (
          <div style={{display:'flex',flexDirection:'column',gap:0,maxHeight:260,overflowY:'auto',scrollbarWidth:'none'}}>
            {repos.map((r,ri) => {
              const isActive = activeRepo === r.name
              const langColor = r.language==='Python'?'#4b8bbe':r.language==='C++'?'#f34b7d':r.language==='JavaScript'?'#f1e05a':r.language==='TypeScript'?'#3178c6':r.language==='Rust'?'#dea584':'#00ffc8'
              const sparkMax  = Math.max(...(r.sparkline||[0]),1)
              return (
                <div key={r.name} style={{borderBottom:'1px solid rgba(0,212,255,0.07)',paddingBottom:8,paddingTop:ri===0?0:8}}>
                  {/* Repo header row — clickable */}
                  <div
                    onClick={() => setActiveRepo(isActive ? null : r.name)}
                    style={{display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer',marginBottom:4}}
                  >
                    <div style={{display:'flex',alignItems:'center',gap:7}}>
                      <span style={{fontFamily:'Share Tech Mono',fontSize:8,color:'rgba(0,212,255,0.35)'}}>
                        {isActive ? '▾' : '▸'}
                      </span>
                      <span style={{fontFamily:'Orbitron',fontSize:9.5,color:'rgba(0,200,255,0.95)',letterSpacing:1,textTransform:'uppercase',textShadow:'0 0 8px rgba(0,200,255,0.3)'}}>
                        {r.name}
                      </span>
                    </div>
                    <span style={{fontFamily:'Share Tech Mono',fontSize:8,color:langColor,letterSpacing:0.5}}>
                      {r.language||'—'}
                    </span>
                  </div>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
                    <span style={{...S.dimText,fontSize:8.5}}>PUSHED {r.pushed}</span>
                    <div style={{display:'flex',gap:10}}>
                      <span style={{...S.dimText,fontSize:8.5}}>★ {r.stars}</span>
                      <span style={{...S.dimText,fontSize:8.5}}>⑂ {r.forks}</span>
                      {r.issues>0 && <span style={{fontFamily:'Share Tech Mono',fontSize:8.5,color:'#ffb900'}}>⚠ {r.issues}</span>}
                    </div>
                  </div>

                  {/* Sparkline — 7-day commit activity */}
                  {r.sparkline && r.sparkline.some(v=>v>0) && (
                    <div style={{display:'flex',alignItems:'flex-end',gap:2,height:18,marginBottom:4}}>
                      {r.sparkline.map((v,i) => (
                        <div
                          key={i}
                          ...
                          style={{
                            flex:1,
                            height: v===0 ? 2 : Math.max(3, Math.round((v/sparkMax)*18)),
                            background: v===0
                              ? 'rgba(0,212,255,0.08)'
                              : i===6
                                ? '#00ffc8'
                                : `rgba(0,212,255,${0.3+0.5*(v/sparkMax)})`,
                            borderRadius:1,
                            transition:'height 0.4s ease',
                            boxShadow: v>0 ? `0 0 4px rgba(0,212,255,${0.2+0.3*(v/sparkMax)})` : 'none'
                          }}
                        />
                      ))}
                      <span style={{...S.dimText,fontSize:7.5,marginLeft:4,lineHeight:'18px',whiteSpace:'nowrap'}}>7D</span>
                    </div>
                  )}
                  ...
                  {isActive && (
                    <div style={{marginTop:6,paddingTop:6,borderTop:'1px solid rgba(0,212,255,0.08)'}}>
                      {r.description && (
                        <div style={{...S.dimText,fontSize:8,marginBottom:6,fontStyle:'italic',opacity:0.7}}>
                          {r.description}
                        </div>
                      )}
                        ...
                        <div style={{display:'flex',flexDirection:'column',gap:5}}>
                          {r.commits.map((c,ci) => (
                            <div key={ci} style={{display:'flex',gap:7,alignItems:'flex-start'}}>
                              <span style={{fontFamily:'Share Tech Mono',fontSize:7.5,color:'rgba(0,255,200,0.45)',marginTop:1,flexShrink:0}}>
                                {c.sha}
                              </span>
                              <div style={{flex:1,minWidth:0}}>
                                <div style={{fontFamily:'Share Tech Mono',fontSize:8,color:'rgba(0,212,255,0.85)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                                  {c.message}
                                </div>
                                <div style={{...S.dimText,fontSize:7.5,marginTop:1}}>
                                  {c.author} · {c.time}
                                </div>
                              </div>
                            </div>
